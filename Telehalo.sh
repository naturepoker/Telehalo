...
echo "                        (%                        "
echo "                                                  "
echo "         Binomica Labs - TeleHalo v. 0.01         "
echo "                                                  "
...
#Screen preformatted halobacteria pfam output for similar protein acccession numbers
...
zcat allhalo-pf34.pfam.gz | grep -i $input_pfam | awk '{print $2}' > "$1"_similar_protein_accessions.acc && echo "Protein accessions list created" || echo "Protein accession list compilation failed" 
...
zgrep -f "$1"_similar_protein_accessions.acc halo-prot.accession2taxid.gz | awk '{print $2"\t"$3}' > "$1"_similar_proteins_to_uids && echo "Protein accessions to taxonomic ID list completed" || echo "Protein accessions to taxonomic ID list failed"
cp "$1"_similar_proteins_to_uids TP_output/

#Screening input_proteins_to_uids file to isolate uid for taxonomy tagging

cat "$1"_similar_proteins_to_uids | awk '{print $2}' > "$1"_halobacteria_with_similar_proteins.uid && echo "Halobacteria taxonomic ID list completed" || echo "Halobacteria taxonomic ID list compilation failed"
cp "$1"_halobacteria_with_similar_proteins.uid TP_output/

#Using Taxonkit to link NCBI uid with taxonomic names for identified Halobacteria
#Also creating a temporary linear uid file to create taxonomic tree list

taxonkit lineage "$1"_halobacteria_with_similar_proteins.uid > "$1"_halobacteria-taxonomic-full.txt && echo "Halobacteria taxonomic name list completed" || echo "Halobacteria taxonomic name compilation failed"
taxonkit lineage -t "$1"_halobacteria_with_similar_proteins.uid > "$1"_halobacteria-taxonomic-lineage.txt && echo "Halobacteria taxonomic lineage list completed" || echo "Halobacteria taxonomic lineage compilation failed"

cp "$1"_halobacteria-taxonomic-full.txt "$1"_halobacteria-taxonomic-lineage.txt TP_output/


#Clean up for the working directory

rm "$1"_input_protein_pfam.txt "$1"_similar_protein_accessions.acc "$1"_similar_proteins.seq "$1"_similar_proteins_cds.seq "$1"_similar_proteins_annotation.txt "$1"_similar_proteins_to_uids "$1"_halobacteria_with_similar_proteins.uid "$1"_halobacteria-taxonomic-full.txt "$1"_halobacteria-taxonomic-lineage.txt "$1"_similar_proteins.aln "$1"_similar_proteins.tre && echo "All Telehalo processes completed. Have a nice day!" || echo "Telehalo process terminated with error" 
